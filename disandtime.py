...
脚本功能：
    ...
    4. 按 (Run 区间, Remaining) 分组，计算平均 SimTime(s)。
    5. 绘制散点图：横轴为平均 SimTime(s)，纵轴为 Remaining，不同 Run 区间用不同颜色区分。
    6. 保存图像到 ./plots/avg_simtime_vs_remaining_by_run.png。
"""
...
def load_and_concat(pattern):
    ...
    for fp in files:
        ...
        df_list.append(df[[COL_RUN, 'file_seed', COL_SIMTIME, COL_REMAIN, 'interval']])
...
def compute_avg_simtime(df):
    ...
    return (
        df
        .groupby([COL_RUN, COL_REMAIN], as_index=False)[COL_SIMTIME]
        .mean()
...
def plot_scatter(df_avg, intervals):
    ...
    plt.title('不同 R 区间下 平均 SimTime(s) vs Remaining（散点图）')
    plt.legend(title='R 区间')
    plt.grid(True, linestyle='--', alpha=0.3)
    plt.tight_layout()
    out_path = os.path.join(OUTPUT_DIR, 'avg_simtime_vs_remaining_by_run.png')
...
def main():
    ...
    print(f"共加载 {len(df_all)} 条记录，覆盖的 Run 区间：{df_all[COL_RUN].unique()}")
